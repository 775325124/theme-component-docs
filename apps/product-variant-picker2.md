...
|------|------------------------------|----------|------|------|
| `customDiscountTitle` | 弹层优惠档位标题 | 单行文本 | 是 | 弹层各档标题，逗号分隔。例：`2足購入で,3足購入で,4足購入で` |
| `custom_discount_variant_num` | 优惠档位购买件数 | 单行文本 | 是 | 各档满几件，须与 Shopline 折扣件数一致，逗号分隔。例：`2,3,4` |
| `custom_discount_badge_labels` | 弹层优惠档位角标 | 单行文本 | 否 | 各档角标文案，逗号分隔。例：`最划算,お得,` |
