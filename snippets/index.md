...
技术维护者用的代码片段库。**按场景组织**——同一个场景下，如果 Slate 2.x 和 3.x 都需要写法，会在同一篇文章里用代码切换标签同时给出。

> 这里只是**文档记录**，不是自动注入。真要在某个店铺生效，仍然需要技术同事把对应代码手写到主题文件里（或者通过店铺后台的"自定义代码"功能粘进去，如果你的 Shopline 套餐支持的话）。

## 片段索引

| 场景 | 一句话用途 | 适用版本 | 首次客户/主题 | 详情 |
|------|-----------|---------|-------------|------|
...
2. 填顶部「元信息」徽章 + 用途 + 代码（用 `::: code-group` 给出 2.x/3.x 写法）
3. 回到本页表格末尾追加一行
...
- **2.x 和 3.x 同场景**就放一篇文章里，文章顶部徽章标"两版都有"
